  ...
  try {
    ...
    const result = await query(
      `SELECT ft.*, u.username as author_name,
       ...
       WHERE ft.category_id = $1 AND ft.household_id = $2
...
router.post('/threads', [
  ...
  try {
    ...
    const result = await query(
      `INSERT INTO forum_threads (category_id, household_id, author_id, title, content) 
       VALUES ($1, $2, $3, $4, $5) 
       RETURNING *, (SELECT username FROM users WHERE id = $3) as author_name`,
      [categoryId, req.user.household_id, req.user.id, title, content]
    );
